...
- 全部生成任务及独立资源接口共享进程内搜索并发上限，默认 2（`ZHIJING_SEARCH_CONCURRENCY`，允许 1–8）；排队也计入整体期限。请求失败或取消结算后归还名额。
- 知乎搜索接口对瞬时并发敏感（实测并发 2 稳定，并发 3 起返回 `UPSTREAM_RATE_LIMITED`）。`UPSTREAM_RATE_LIMITED` 会退避重试，最多 3 次尝试，退避期间受总期限取消；其他错误码不重试，直接产出节点错误。重试耗尽后仍返回 `UPSTREAM_RATE_LIMITED`。
